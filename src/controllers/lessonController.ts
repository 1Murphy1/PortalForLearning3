...
const createLesson = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const newLesson = new LessonModel({ title, content, videoUrl, course, order });
...
const updateLesson = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const updatedLesson = await LessonModel.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
    if (!updatedLesson) {
      res.status(404).json({ message: 'Lesson not found' });
      return;
    }
...
const getLessonWithComments = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const lesson = await LessonModel.findById(id).populate('course').populate('comments');
...
export const lessonController = { getLessons, getLessonById, createLesson, updateLesson, deleteLesson, getLessonWithComments };
